...
```powershell
powershell -ExecutionPolicy ByPass -c "irm https://uvx.sh/confluence-markdown-exporter/5.2.0/install.ps1 | iex"
```
...
```bash
pip install confluence-markdown-exporter==5.2.0
```
...
```bash
uv tool install confluence-markdown-exporter==5.2.0
```
...
```bash
docker pull spenhouet/confluence-markdown-exporter:5.2.0
```
